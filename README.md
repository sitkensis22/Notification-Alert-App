...
**Set tag prelease days (`tag_prerelease_days`):** This integer input defines the number of days before the scheduled release date to generate an alert. When the collar or tag has resurrected longer than this duration after a period of non-transmission, the alert will be activated. Note that the input will only be used when tag scheduled detachment alert is activated.
